...
import com.nononsenseapps.feeder.util.TimeUtils
...
fun ArticleScreen(
    ...
    ArticleScreen(
        ...
        onAddToCalendar = {
            // 提取文章标题和内容
            val articleText = "${viewState.articleTitle} ${viewState.articleContent}"

            // 使用我们的工具解析时间
            val timeRange = TimeUtils.extractEventTimeRangeEnhancedLegacy(articleText)

            // 创建日历Intent
            val intent = Intent(Intent.ACTION_INSERT).apply {
                data = android.provider.CalendarContract.Events.CONTENT_URI
                putExtra(android.provider.CalendarContract.Events.TITLE, viewState.articleTitle)
                putExtra(android.provider.CalendarContract.Events.DESCRIPTION, viewState.articleLink ?: "")

                // 如果成功提取到时间，则设置开始和结束时间
                timeRange.first?.let { startTime ->
                    putExtra(android.provider.CalendarContract.EXTRA_EVENT_BEGIN_TIME, startTime)
                }
                timeRange.second?.let { endTime ->
        ...
        topBar = {
            SensibleTopAppBar(
                ...
                actions = {
                    ...
                    PlainTooltipBox(tooltip = { Text("Add to Calendar") }) {
                        IconButton(
